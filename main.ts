...
export class HealthMonitor {
  ...
  /**
   * Get detailed health metrics including error rate and performance stats
   */
  getDetailedHealthMetrics(): {
    ...
    const total = this.operationCounts.successful + this.operationCounts.failed;
    const errorRate = total > 0 ? (this.operationCounts.failed / total) * 100 : 0;
    
    return {
      health,
      errorRate,
      averageOperationTime: metricsCollector.getAverageExecutionTime(),
      lastOperationTime: Date.now() - this.startTime
    };
  }
...
export function main(): void {
  ...
  // Display health status (with detailed metrics if enabled)
  if (config.enableDetailedHealthMetrics) {
    const detailedMetrics = healthMonitor.getDetailedHealthMetrics();
    console.log('\n=== Detailed Health Metrics ===');
    console.log(`Status: ${detailedMetrics.health.status}`);
    console.log(`Uptime: ${detailedMetrics.health.uptime}ms`);
    console.log(`Operations: ${detailedMetrics.health.operations.successful}/${detailedMetrics.health.operations.total} successful`);
    console.log(`Error Rate: ${detailedMetrics.errorRate.toFixed(2)}%`);
    console.log(`Average Operation Time: ${detailedMetrics.averageOperationTime.toFixed(2)}ms`);
    console.log(`Memory usage: ${detailedMetrics.health.memory.used}MB (${detailedMetrics.health.memory.percentage.toFixed(1)}%)`);
    console.log('=== End Detailed Health Metrics ===\n');
  } else {
...
export interface AppConfig {
  ...
  enableDetailedHealthMetrics: boolean;
